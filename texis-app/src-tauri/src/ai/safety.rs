...
impl AiSafetyPolicy {
    ...
    pub fn classify_mode(mode: &AiActionMode) -> AiSafetyDecision {
        match mode {
            ...
            AiActionMode::Ask => AiSafetyDecision::low("Pregunta libre — sin cambios"),
            ...
            AiActionMode::LearnLatex => AiSafetyDecision::low("Explicación de LaTeX — sin cambios"),
            ...
            AiActionMode::ImproveWriting => AiSafetyDecision::auto_notify(
                "Edita texto existente — se notifica y se puede deshacer",
            ),
            AiActionMode::ShortenText => AiSafetyDecision::auto_notify(
                "Edita texto existente — se notifica y se puede deshacer",
            ),
            AiActionMode::ExpandText => AiSafetyDecision::auto_notify(
                "Edita texto existente — se notifica y se puede deshacer",
            ),
            AiActionMode::RewriteText => AiSafetyDecision::auto_notify(
                "Edita texto existente — se notifica y se puede deshacer",
            ),
            AiActionMode::ConvertToLatex => AiSafetyDecision::auto_notify(
                "Convierte selección a LaTeX — se notifica y se puede deshacer",
            ),
            AiActionMode::AddParagraph => AiSafetyDecision::medium(
                "Añade párrafo nuevo — requiere preview y confirmación antes de insertarse",
            ),
            ...
            AiActionMode::InsertTable => AiSafetyDecision::medium(
                "Inserta tabla nueva — requiere revisar estructura y datos",
            ),
            AiActionMode::InsertFigurePlaceholder => AiSafetyDecision::medium(
                "Inserta figura placeholder — requiere confirmar caption y label",
            ),
            AiActionMode::InsertEquation => AiSafetyDecision::medium(
                "Inserta ecuación — requiere verificar la expresión matemática",
            ),
            AiActionMode::AddGlossaryEntry => AiSafetyDecision::medium(
                "Añade término al glosario — requiere confirmar definición",
            ),
            AiActionMode::AddAcronym => {
                AiSafetyDecision::medium("Añade acrónimo — requiere confirmar forma corta y larga")
            }
            AiActionMode::InsertCodeBlock => AiSafetyDecision::medium(
                "Inserta bloque de código — requiere verificar el contenido",
            ),
            AiActionMode::GenerateAbstract => AiSafetyDecision::medium(
                "Propone abstract — el autor debe revisar antes de insertar",
            ),
            AiActionMode::GenerateCaption => AiSafetyDecision::medium(
                "Propone caption — el autor debe verificar antes de insertar",
            ),
        }
    }
    ...
    pub fn classify_action(action: &AiProposedAction, mode: &AiActionMode) -> AiSafetyDecision {
        match action {
            AiProposedAction::ShowInChat { .. } => AiSafetyDecision::low("Solo muestra en chat"),

            AiProposedAction::ReplaceSelection {
                original,
                replacement,
                ..
            } => {
    ...
    pub fn validate_response_text(text: &str) -> Result<(), String> {
        ...
        for pattern in &forbidden {
            if lower.contains(pattern) {
                return Err(format!(
                    "Respuesta bloqueada: contiene patrón peligroso '{}'",
                    pattern
                ));
            }
        }
...
mod tests {
    ...
    fn ask_is_low() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::Ask).risk_level,
            AiRiskLevel::Low
        );
    }
    #[test]
    fn explain_error_is_low() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::ExplainLatexError).risk_level,
            AiRiskLevel::Low
        );
    }
    #[test]
    fn simulate_examiner_is_low() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::SimulateExaminer).risk_level,
            AiRiskLevel::Low
        );
    }
    #[test]
    fn review_content_is_low() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::ReviewContent).risk_level,
            AiRiskLevel::Low
        );
    }
    #[test]
    fn suggest_sources_is_low() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::SuggestSources).risk_level,
            AiRiskLevel::Low
        );
    }
    ...
    // AutoWithNotification
    #[test]
    fn improve_writing_is_auto_notify() {
    ...
    #[test]
    fn shorten_is_auto_notify() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::ShortenText).risk_level,
            AiRiskLevel::AutoWithNotification
        );
    }
    #[test]
    fn expand_is_auto_notify() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::ExpandText).risk_level,
            AiRiskLevel::AutoWithNotification
        );
    }
    #[test]
    fn add_paragraph_requires_confirmation() {
    ...
    #[test]
    fn insert_citation_requires_confirmation() {
    ...
    #[test]
    fn add_bibliography_entry_requires_confirmation() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::AddBibliographyEntry).risk_level,
            AiRiskLevel::Medium
        );
    }
    #[test]
    fn insert_table_requires_confirmation() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::InsertTable).risk_level,
            AiRiskLevel::Medium
        );
    }
    #[test]
    fn insert_figure_requires_confirmation() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::InsertFigurePlaceholder).risk_level,
            AiRiskLevel::Medium
        );
    }
    #[test]
    fn generate_abstract_requires_confirmation() {
        assert_eq!(
            AiSafetyPolicy::classify_mode(&AiActionMode::GenerateAbstract).risk_level,
            AiRiskLevel::Medium
        );
    }
    ...
    #[test]
    fn empty_selection_is_forbidden() {
        let action = AiProposedAction::ReplaceSelection {
            original: String::new(),
    ...
    #[test]
    fn blocks_structural_latex_commands() {
