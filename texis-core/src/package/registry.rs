...
impl PackageRegistry {
    ...
    pub fn register(&mut self, name: &str, options: Vec<String>, source: EntrySource) {
        ...
        self.packages.entry(name.to_string()).or_insert_with(|| RegistryEntry {
            name: name.to_string(),
            options,
            priority,
            source,
    ...
    pub fn update_from_analysis(&mut self, analysis: &PackageAnalysis) {
        for req in &analysis.missing {
            self.packages.entry(req.package_name.clone()).or_insert_with(|| RegistryEntry {
        ...
        for declared in &analysis.declared {
            self.packages.entry(declared.clone()).or_insert_with(|| RegistryEntry {
                name: declared.clone(),
                options: vec![],
    ...
    pub fn detect_conflicts(&self) -> Vec<KnownConflict> {
        let known: &[(&str, &str, &str)] = &[
            ("babel", "polyglossia", "Son mutuamente excluyentes; usa uno de los dos."),
            ("subfig", "subcaption", "Conflicto de definiciones; usa subcaption."),
            ("epsfig", "graphicx", "epsfig está obsoleto; usa solo graphicx."),
        ];
...
mod tests {
    ...
    fn save_and_load_roundtrip() {
        ...
        reg.register("pgfplots", vec!["compat=1.18".to_string()], EntrySource::User);
