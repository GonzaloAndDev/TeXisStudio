...
impl DiagnosticRegistry {
    ...
    pub fn replace_source(&mut self, source: &DiagnosticSource, new: Vec<Diagnostic>) {
        ...
        let old_ids: Vec<DiagnosticId> = self
            .by_source
            .remove(&source_str)
            .unwrap_or_default();
        ...
        for d in new { self.push(d); }
    }

    pub fn all(&self) -> &[Diagnostic] { &self.diagnostics }
    ...
    pub fn warnings(&self) -> Vec<&Diagnostic> {
        self.diagnostics.iter().filter(|d| d.severity == DiagnosticSeverity::Warning).collect()
    }
    ...
    pub fn has_blocking_errors(&self) -> bool { self.diagnostics.iter().any(|d| d.is_blocking) }
}
...
mod tests {
    ...
    fn push_and_count() {
        ...
        reg.push(Diagnostic::error(DiagnosticSource::LatexLog, "E_TEST", "test error"));
    ...
    fn replace_source_clears_old() {
        let mut reg = DiagnosticRegistry::new();
        reg.push(Diagnostic::error(DiagnosticSource::LatexLog, "E_OLD", "old"));
        reg.replace_source(&DiagnosticSource::LatexLog, vec![
            Diagnostic::warning(DiagnosticSource::LatexLog, "W_NEW", "new"),
        ]);
        assert_eq!(reg.total(), 1);
    ...
    fn has_blocking_errors_correct() {
        let mut reg = DiagnosticRegistry::new();
        reg.push(Diagnostic::warning(DiagnosticSource::LatexLog, "W_X", "warn"));
        assert!(!reg.has_blocking_errors());
        reg.push(Diagnostic::error(DiagnosticSource::LatexLog, "E_X", "error"));
        assert!(reg.has_blocking_errors());
    }
}
