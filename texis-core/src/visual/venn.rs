...
fn set_color(color: &str, idx: usize) -> &str {
    ...
    match color {
        ...
        "teal"  | "#1ABC9C" => "teal",
        _ => match idx { 0 => "red", 1 => "blue", 2 => "green", _ => "orange" },
    }
}
...
fn render_2(c: &VennEulerConfig) -> String {
    ...
    let inter_01 = c.intersections.get("01").map(|s| latex_escape(s)).unwrap_or_default();

    format!(r#"\begin{{tikzpicture}}[scale=1.1]
...
fn render_3(c: &VennEulerConfig) -> String {
    let sets: Vec<_> = (0..3)
        .map(|i| c.sets.get(i).cloned().unwrap_or_else(|| crate::project::model::VennSet {
    ...
    let colors: Vec<_> = sets.iter().enumerate().map(|(i,s)| set_color(&s.color, i)).collect();

    // Intersecciones: 01=AB, 02=AC, 12=BC, 012=ABC
    let i01 = c.intersections.get("01").map(|s| latex_escape(s)).unwrap_or_default();
    let i02 = c.intersections.get("02").map(|s| latex_escape(s)).unwrap_or_default();
    let i12 = c.intersections.get("12").map(|s| latex_escape(s)).unwrap_or_default();
    let i012= c.intersections.get("012").map(|s| latex_escape(s)).unwrap_or_default();
...
\end{{tikzpicture}}"#,
        ...
        l0=labels[0], l1=labels[1], l2=labels[2],
        i01=i01, i02=i02, i12=i12, i012=i012,
    )
}
...
fn render_4(c: &VennEulerConfig) -> String {
    ...
    format!(r#"\begin{{tikzpicture}}[scale=1.0]
