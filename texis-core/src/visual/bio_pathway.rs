...
fn krebs_cycle(c: &BioPathwayConfig) -> String {
    ...
    format!(r#"\usetikzlibrary{{shapes.geometric,calc}}
\begin{{tikzpicture}}[
...
\end{{tikzpicture}}"#,
  ...
  \node[font=\scriptsize,text=green!60!black]  at (125:\R*1.2) {NADH};"# } else { "" },
    )
}
...
fn glycolysis(c: &BioPathwayConfig) -> String {
    ...
    for (i, (id, lbl)) in steps.iter().enumerate() {
        let pos = if i == 0 { String::new() } else { format!("[below of={}]", steps[i-1].0) };
        out.push_str(&format!("  \\node[metabolite] ({}) {} {{{}}};\n", id, pos, lbl));
        if i > 0 {
            let cof = if show { cofactors_right[i] } else { "" };
            let cof_node = if !cof.is_empty() {
                format!(" node[right=0.1cm, font=\\scriptsize, text=purple] {{\\scriptsize {}}}", cof)
            } else { String::new() };
            out.push_str(&format!("  \\draw[->] ({}) --{} ({});\n", steps[i-1].0, cof_node, id));
        }
    }
...
fn photosynthesis(c: &BioPathwayConfig) -> String {
    format!(r#"\begin{{tikzpicture}}[scale=0.9, node distance=1.4cm]
...
fn electron_transport(c: &BioPathwayConfig) -> String {
    ...
    for (i, (lbl, color)) in steps.iter().enumerate() {
        let pos = if i == 0 { String::new() } else { format!("[right of=c{}]", i) };
        out.push_str(&format!("  \\node[complex, fill={}] (c{}) {} {{{}}};\n", color, i+1, pos, lbl));
    }
...
fn beta_oxidation(c: &BioPathwayConfig) -> String {
    ...
    let cofactors = if c.show_cofactors { ["", "FAD→FADH$_2$", "H$_2$O", "NAD$^+$→NADH", "CoA-SH"] } else { ["","","","",""] };
    let mut out = String::from("\\begin{tikzpicture}[node distance=1.3cm, font=\\small]\n");
    ...
    for (i, lbl) in steps.iter().enumerate() {
        let pos = if i == 0 { String::new() } else { format!("[below of=s{}]", i) };
        out.push_str(&format!("  \\node[step] (s{}) {} {{{}}};\n", i+1, pos, lbl));
        if i > 0 {
            let cf = cofactors[i];
            let cf_node = if !cf.is_empty() { format!(" node[right=0.05cm, font=\\scriptsize] {{\\scriptsize {}}}", cf) } else { String::new() };
            out.push_str(&format!("  \\draw[->] (s{}) --{} (s{});\n", i, cf_node, i+1));
        }
    }
    out.push_str("\\end{tikzpicture}");
    out
