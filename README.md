...
El primer arranque es un recorrido de dos pasos: primero se elige el idioma y, a continuación, **cómo quieres trabajar** — **Principiante** (recomendado: escribir sin saber LaTeX, con editores visuales, plantillas curadas y redes de seguridad) o **Experto** (control total: interioridades de LaTeX, registros técnicos, varios motores, plugins avanzados y edición cruda). Esa elección fija el `userMode`, que gobierna toda la superficie de complejidad de la app (barra del editor, log técnico de compilación, paleta de comandos, secciones de Ajustes), y siempre se puede cambiar después en Ajustes.

Al crear un proyecto, el asistente ofrece (activado por defecto) **incluir contenido de ejemplo**: siembra cada sección del cuerpo con un párrafo inicial —y la introducción con una lista de objetivos y una ecuación numerada de ejemplo— para que la **primera compilación produzca un PDF real** que el principiante puede editar, en vez de un documento casi vacío con metadatos de relleno. El contenido es prosa del *documento* (localizada al idioma del documento, no al de la interfaz) y se siembra en el mismo paso de guardado del asistente, sin tocar la ruta de creación del backend; los expertos pueden desactivarlo para empezar en blanco.
...
La vista de progreso incluye además una pestaña **Cambios desde la última revisión**: al descargar el reporte para el asesor (o pulsar «Marcar revisión») se captura un punto de referencia ligero —por sección: palabras, número de bloques y estado editorial— y a partir de ahí se muestra qué cambió (secciones nuevas, ampliadas, reducidas, eliminadas, o con transición de estado, con el delta de palabras). Así el autor y su asesor ven de un vistazo qué se movió entre revisiones. El punto de referencia se guarda por proyecto en el almacenamiento local del frontend, sin tocar el pipeline transaccional de guardado.
